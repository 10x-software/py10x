...
### Added
- **Traitable store-side trait flags `TS_TIME` / `TS_USER` / `TS`** (py10x-kernel `btrait.h` / `btraitable.cpp` / `py10x_kernel.cpp`; `core_10x/traitable.py`, `core_10x/trait.py`): exclusive store-populated kinds (`TS_TIME`, `TS_USER`) with `TS = TS_TIME|TS_USER` as the any-store-side mask — omitted from the client `serialize_traits` blob, filled by default `Traitable.post_serialize` via `TsStore.add_when` / `add_who`, and loaded on deserialize/reload. Exact kind checks use `(flags & TS) == TS_TIME` / `TS_USER`. `TS_TIME` requires `datetime`; `TS_USER` requires `str`; traits must be storable. `Traitable.runtime()` clears `TS`. `EventBase._at` / `_who` use `T(T.TS_TIME)` / `T(T.TS_USER)`. Store `add_who` writes the given field name (Mongo/DuckDB).
- **Hydrate store-side traits after `save()` without reload** (`core_10x/traitable.py`, `core_10x/ts_store.py`, Mongo/DuckDB collections): `TsCollection.save` / `save_new` return `{_rev, ...store fields present post-write}`; `StorableHelper` passes candidate `ts_trait_names` from `traits(flags_on=T.TS)` and applies only keys present in the result (custom `post_serialize` may skip `add_when` / `add_who` for some traits). No side-channel key in the write blob (field names come from flags).
