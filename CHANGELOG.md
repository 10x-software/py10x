...
- **Environment variables** (`core_10x/environment_variables.py`):
  ...
  - **`vault_ts_store_uri` default**: now falls back to `main_ts_store_uri` via a `vault_ts_store_uri_get` method when `XX_VAULT_TS_STORE_URI` is unset.
