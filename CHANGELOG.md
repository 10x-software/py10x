...
### Changed
- **Package and branding**: C++ packages renamed to `py10x_core` and `py10x_infra` (from core_10x_i / infra_10x_i); 10X Concepts LLC → 10X CONCEPTS LLC; email domain updates
...
- **0.1.4**: Subclasses of `Traitable` must not override `__init__`; use `__post_init__` for customization. Python 3.10 no longer supported (3.11+ required). C++ packages are now `py10x_core` and `py10x_infra` (rename from core_10x_i / infra_10x_i if you referenced them directly).
