...
- **Storage / Ibis**: `TraitableHistory.traitable_get` handles a missing `_collection_name` when stripping the `#history` suffix; `DuckDbStore.server_time` uses `datetime.utcnow()`; composite and null-valued trait filters translate correctly for DuckDB-backed Ibis queries.
...
- **UI tests**: reduced Rio flakiness on slow CI via `browser_helpers` condition waits and longer collection-editor timeouts.
