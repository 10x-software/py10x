...
- **Storage and Ibis querying** (`infra_10x/ibis_store.py`, `infra_10x/duckdb_store.py`, `infra_10x/postgres_store.py`, `core_10x/trait.py`, `core_10x/concrete_traits.py`): shared `IbisStore` / `IbisCollection` hybrid columns + JSON blob, trait-aware filter pushdown via `trait_filter.ibis()`, and Mongo-style `new_instance(hostname, dbname, username, password, **kwargs)`; `DuckDbStore` in-memory test store; **`PostgresStore`** (`TS_STORE_TYPE.POSTGRESQL`) with JSONB `_data`, `ON CONFLICT` upsert, long-name physical-table hashing (PG 63-byte limit), and **server-side** `TS_USER` via SQL `current_user` (not Resource username — unlike Mongo/DuckDB). CI provisions Postgres (`.github/actions/setup-postgres`, OS-user trust); infra shared suites matrix Mongo + Postgres (`need()` soft for Postgres, hard Mongo connect). `Trait.serialize_to_types` / `s_serialize_to_type` declare wire serialization types (e.g. `datetime_trait` → `str`) for pushdown and deserialization.
