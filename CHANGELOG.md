...
- **Hydrate store-side traits after `save()` without reload** (`core_10x/traitable.py`, `core_10x/ts_store.py`, Mongo/DuckDB collections): `post_serialize` records intent via `TsStore.add_ts` → `_ts_fields`; `save` / `save_new` apply server stamps and return `{_rev, ...stamped fields}`; client applies only keys present in the result. Replaces materializing `add_when` / `add_who`.
...
- **Storage and Ibis querying** (`infra_10x/duckdb_store.py`, `infra_10x/ibis_store.py`, `core_10x/trait.py`, `core_10x/concrete_traits.py`): `DuckDbStore` in-memory test store with shared `IbisStore` / `IbisCollection` read-path logic and trait-aware filter pushdown via `trait_filter.ibis()`; `Trait.serialize_to_types` / `s_serialize_to_type` declare wire serialization types (e.g. `datetime_trait` → `str`) for pushdown and deserialization.
