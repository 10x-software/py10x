...
### Changed
- **Embeddable values on non-embeddable traitable traits** (`core_10x/traitable.py` `traitable_trait.serialize`): serializing an embeddable instance (`s_embeddable`, e.g. `AnonymousTraitable`) through a trait whose declared `data_type` is **not** embeddable now raises `RuntimeError`. Previously the value was written as an embedded body regardless of the declared type. Embed bodies are only produced when the trait’s `data_type` is embeddable (`embeddable=True` / `AnonymousTraitable`); non-embeddable declared types store by reference only.
