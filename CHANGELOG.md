...
### Added
- **Python 3.13**: `requires-python` is now `>=3.11,<3.14`; classifiers and the CI matrix cover 3.11, 3.12, and 3.13.
- **`py10x-fin-base` / `py10x-fin-base-cxx`** (`xx_fin/`): first promote **downstream** of `py10x-core`. Consumers install **`py10x-fin-base` only** (imports `xxfin` / `cxxfin`; cxx is transitive). Core does **not** depend on fin-base. Requires Python `>=3.11,<3.14`. CI asserts `xxfin` is absent before the core suite, then `uv-sync --with-downstream` and runs `xx_fin/` tests. See [`xx_fin/README.md`](xx_fin/README.md).
...
- **`Traitable.from_id(_id)`** and **`load` / `load_many(..., reload=True)`**: construct a traitable from an existing `ID`; `reload=False` reuses a cached instance instead of re-deserializing from the store (default remains reload).
- **Release engineering and CI** (`dev_10x/`, `.github/`): coordinated releases across py10x-core and both C++ siblings — `xx-promote` cuts rc/final releases onto tool-owned `pre`/`prod` branches with `==`-pinned siblings, yanks tags, and reports publish state, using isolated publish-trigger tags and an idempotent planner with `resync` recovery. `uv-sync` / `uv-run` drive dependency-source profiles; `constraints.txt` + `xx-constraints` freeze the third-party graph. Reusable composite actions, a Windows CI matrix, a nightly ASan lane, and a pre-publish test gate. Live-store tests get their own database per session, with `xx-test-db-clean` for strays. Operational detail and agent guardrails: [`dev_10x/README.md`](dev_10x/README.md).
- **Storage and Ibis querying** (`infra_10x/ibis_store.py`, `duckdb_store.py`, `postgres_store.py`): shared `IbisStore` / `IbisCollection` — hybrid SQL columns plus a JSON blob, with trait-aware filter pushdown via `trait_filter.ibis()`; `DuckDbStore` in-process; **`PostgresStore`** (`TS_STORE_TYPE.POSTGRESQL`) with JSONB `_data`, `ON CONFLICT` upsert, long-name table hashing (PG 63-byte limit) recorded as a table comment, and **server-side** `TS_USER` from SQL `current_user`. Filters now match MongoDB on every backend, including structure-valued (`dict` / `list`) and `None`-in-`IN`/`NIN` cases. `collection_names` is catalog-backed, so a store-wide `copy_to` sees collections written by an earlier process; blob-only collections copy rather than abort. `Traitable.delete_collection(..., drop_history=True)` also drops the `#history` companion. CI provisions Postgres and the shared infra suites run Mongo + Postgres.
...
- **`Traitable.__init__` is keyword-only** (breaking): `Cls(_id)` / positional `_collection_name` no longer work. Use `Cls(_id=_id)` or **`Cls.from_id(_id)`**. Trait values were already kwargs.
...
- **UI choices**: `get_choices` / collection-editor / Rio dialog accept traitable choice values so display text can come from the object rather than a bare string.
- **Dependencies and packaging**: new published runtime deps `ibis-framework[postgres]`, `polars`, `adbc-driver-manager`, `uuid6`, `psutil`; Dependabot bumps (`rio-ui` 0.12.2, widened `playwright` and `cryptography` ranges); optional `[project.optional-dependencies] jit` group for JIT tooling (not part of the default install).
...
- **Storage / Ibis**: `TraitableHistory.traitable_get` handles a missing `_collection_name` when stripping the `#history` suffix; empty `IN`/`NIN` filters; `save_new` revision when TS fields are stamped in the same round-trip; history of self-referencing objects.
...
- **Unreleased**: (none yet)
- **0.3.0**: Python 3.13; py10x-fin-base downstream; PostgresStore/Ibis/DuckDB; TS_TIME/TS_USER; T.NOT_EMBEDDABLE; xx-promote/constraints/uv-sync; Event/EventProcessor; keyword-only Traitable.__init__ + from_id; xxcommon rename; testdb→duckdb; experimental JIT/cxx curve
...
- **0.3.0**: **Import rename:** `xx_common` → `xxcommon` (no shim). **Test store URI:** `testdb://` → `duckdb://`; `TS_STORE_TYPE.TESTDB` → `TS_STORE_TYPE.DUCKDB`. **`Traitable.__init__` is keyword-only:** use `Cls.from_id(_id)` or `Cls(_id=_id)` instead of `Cls(_id)`. **`Traitable.save`:** default `save_references` is `BSaveRefs.NEW_ONLY` (was root-only). **`TraitableCli` option syntax:** `option=value` → `--option value` (booleans: `--flag` / `--no-flag`). **`ID.__hash__`:** now includes `collection_name` — IDs with the same value in different collections are no longer equal in sets/dicts. **RT-only traitables:** `_collection_name` is not a trait; instance access raises `AttributeError` (was `XNone`). **`T.NOT_EMBEDDABLE`** added (ref-only traitable traits); **`T.EMBEDDED`** retained for closed embed trees.
