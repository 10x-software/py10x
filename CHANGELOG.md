...
### Added
- **`T.NOT_EMBEDDABLE`** (py10x-kernel `btrait.h` / `py10x_kernel.cpp`; `core_10x/traitable.py` `traitable_trait` only): ref-only flag on scalar traitable traits. Rejects embeddable `data_type` at class definition and embeddable values at serialize time. Mutually exclusive with `T.EMBEDDED`.
- **Traitable store-side trait flags `TS_TIME` / `TS_USER` / `TS`** (py10x-kernel `btrait.h` / `btraitable.cpp` / `py10x_kernel.cpp`; `core_10x/traitable.py`, `core_10x/trait.py`): exclusive store-populated kinds (`TS_TIME`, `TS_USER`) with `TS = TS_TIME|TS_USER` as the any-store-side mask — omitted from the client `serialize_traits` blob, marked by default `Traitable.post_serialize` via `TsStore.add_ts` → `_ts_fields`, applied in collection `save`/`save_new`, and loaded on deserialize/reload. Exact kind checks use `(flags & TS) == TS_TIME` / `TS_USER`. `TS_TIME` requires `datetime`; `TS_USER` requires `str`; traits must be storable. `Traitable.runtime()` clears `TS`. `EventBase._at` / `_who` use `T(T.TS_TIME)` / `T(T.TS_USER)`.
- **Hydrate store-side traits after `save()` without reload** (`core_10x/traitable.py`, `core_10x/ts_store.py`, Mongo/Ibis collections): `post_serialize` records intent via `TsStore.add_ts` → `_ts_fields`; `save` / `save_new` apply server stamps and return `{_rev, ...stamped fields}`; client applies only keys present in the result. Replaces materializing `add_when` / `add_who`.
- **Release engineering and CI** (`dev_10x/`, `.github/`): coordinated release tooling across py10x-core and both C++ siblings — `xx-promote` / `xx_plan` / `xx_helpers` cut rc/final releases onto tool-owned `pre`/`prod` branches with exact `==` sibling pins, roll rc-window pins on `main`, yank tags, and inspect publish state (`pre`, `prod`, `yank`, `status`, `resync`; `--dry-run`, `--push`, `--publish-only`, `--no-publish`); isolated publish-trigger tags (`pre/{release}`, `prod/{release}`) decouple CI webhooks from bundled version-tag pushes; idempotent pure planner with atomic `git push --atomic` per repo, untracked-files-OK promote precondition, and crash recovery via `resync` + re-run; skip rc tagging when there is no diff from the previous tag and block prod until the latest tag is an rc. `xx_ci` (kernel-free) resolves sibling tags for publish CI. `uv-sync` redesign drives `uv pip install` directly (no transient `[tool.uv.sources]` edits); `uv-run` reuses the last sync profile; `constraints.txt` + `xx-constraints` freeze the transitive third-party graph (`compile --upgrade`, `check`); weekly refresh workflow opens PRs when pins drift. `XX_UV_BUILD_TYPE` (`Release`/`Debug`) and `XX_UV_INCREMENTAL` for faster local C++ sibling rebuilds. `pyproject.toml` sibling layout under `[tool.dev_10x.siblings]`; default `[tool.uv.sources]` removed. Reusable composite actions, Windows CI matrix, cxx10x branch resolution on PRs (`.github/scripts/resolve-sibling-branch.sh`), pre-publish test gate in `build.yml`. `CODEOWNERS`; cloud setup scripts (`scripts/cloud_setup.sh`, `scripts/cloud_mongo.sh`). Each pytest session creates and drops its **own** database on every live backend via the requested-not-autouse `live_store` fixture (`core_10x/testlib/test_databases.py`; `instance_from_uri(..., _create_if_needed=True)` plus `TsStore.list_databases` / `delete_database` — Postgres, unlike Mongo, will not create one on first write), so a run never touches a developer's own data, nothing accumulates between runs, and concurrent sessions cannot delete each other's data; `xx-test-db-clean` drops databases left by a killed session. See `dev_10x/README.md` for operational detail.
- **Storage and Ibis querying** (`infra_10x/ibis_store.py`, `infra_10x/duckdb_store.py`, `infra_10x/postgres_store.py`, `core_10x/trait.py`, `core_10x/concrete_traits.py`): shared `IbisStore` / `IbisCollection` hybrid columns + JSON blob, trait-aware filter pushdown via `trait_filter.ibis()`, and Mongo-style `new_instance(hostname, dbname, username, password, **kwargs)`; `DuckDbStore` in-memory test store; **`PostgresStore`** (`TS_STORE_TYPE.POSTGRESQL`) with JSONB `_data`, `ON CONFLICT` upsert, long-name physical-table hashing (PG 63-byte limit), and **server-side** `TS_USER` via SQL `current_user` (not Resource username — unlike Mongo/DuckDB). CI provisions Postgres (`.github/actions/setup-postgres`: OS-user trust on 5432 + password-auth companion on 5433); infra shared suites matrix Mongo + Postgres (`need()` soft for Postgres, hard Mongo connect). Ibis index names are namespaced per collection (PG schema-global `IF NOT EXISTS` safety). `Trait.serialize_to_types` / `s_serialize_to_type` declare wire serialization types (e.g. `datetime_trait` → `str`) for pushdown and deserialization. Structure-valued filters (`dict` / `list`, and `NamedConstant` subclasses serializing to `_type`/`_cls`/`_obj`) compare as the dialect's own JSON type rather than as rendered text — Postgres declares JSONB, whose equality is semantic, so filters no longer depend on the key order the blob was stored in. `IN` / `NIN` fold `None` out of the SQL `IN` list into an explicit NULL test (under three-valued logic `x IN (a, NULL)` and `x NOT IN (a, NULL)` are never TRUE), and a blob key stored as JSON `null` reads back as SQL NULL on both the scalar and JSON paths — so mixed structure/scalar/`None` lists now match MongoDB exactly on every backend. `trait_dir={}` means "declared, nothing column-eligible" (writable, blob-only) and is no longer conflated with `None` ("no schema", read-only), so a blob-only collection copies instead of aborting a store-wide `copy_to`. `Traitable.delete_collection(..., drop_history=True)` also drops the `#history` companion (opt-in — an audit trail should not be destroyed as a side effect), and the suffix is a named concept on `TraitableHistory` (`s_suffix`, `history_collection_name`, `base_collection_name`) instead of being spelled inline. `IbisStore.collection_names` is **catalog-backed** (was: only collections opened by that store object) so a freshly connected store sees collections written by an earlier process — otherwise store-wide `copy_to` from a persistent dialect silently copied nothing; tables without the `_id` / `_rev` / `_data` layout are skipped, and `PostgresStore` stamps the logical collection name as a table comment so hashed physical names stay resolvable.
- **Traitable and trait framework** (`core_10x/traitable.py`, `core_10x/trait.py`, `core_10x/named_constant.py`; py10x-kernel `btraitable.cpp`, `btrait_processor.cpp`): `cxx_mixins` let subclasses declare extra pybind mixin classes — trait getters resolve from mixins before Python methods; `Trait.pybind_signature` parses pybind11 docstring signatures when `inspect.signature` cannot. **`post_serialize` / `deserialize_traits` (py10x-core + py10x-kernel):** `post_serialize` hook on save for store-side fields (see `TS` / `TS_TIME` / `TS_USER` above); `TraitableHistory` overrides `deserialize_traits` to split history vs event fields; kernel `deserialize_traits` skips valid `EVAL_ONCE` on reload and no longer throws when invalidating unset `EVAL_ONCE` traits. `NamedConstant.__init_subclass__(symbols_from=…)` populates `s_dir` from another class's UPPERCASE attributes. `_collection_name` read path served by C++ (`BTraitable.collection_name`) with Python `_collection_name_set` setter. `ID.__hash__` includes `collection_name`. RT-only traitables: `_collection_name` is not a trait and instance access raises `AttributeError`. `keep_history` / `immutable` on a runtime base propagate to concrete subclasses; history class resolved via `traitable.s_history_class`. **Declarative collection indexes**: `s_indices = [Index(name, spec, **kwargs), ...]` on `Traitable` (and subclasses); `spec` can be a field name or list of `(field, direction)` tuples; indexes ensured only on first write via new `@cache(keep_value=False)` support; `EventBase` and `TraitableHistory` now declare their standard indexes (`_at_idx`, `idx_by_traitable_id_time`) declaratively (instead of imperative `create_index`); indexes inherited from base classes; see `GETTING_STARTED.md`; new `test_global_cache.py`.
- **`TraitableCli` and `exc_to_rc`** (`core_10x/traitable_cli.py`, `core_10x/rc.py`): CLI options use `--option value` (dashes → underscores) with `--option` / `--no-option` boolean shortcuts; `CONVERT_VALUES_ON` enabled automatically during argument parsing. `exc_to_rc` wraps void callables for CLI commands — returns `RC_TRUE` on success or `RC(False, message)` on failure (omitted `message` captures the active exception).
...
- **`Traitable.save(save_references=…)`**: default now also saves never-persisted storable references (`BSaveRefs.NEW_ONLY`). Use `BSaveRefs.ALL` (or `True`) for a full graph save, `BSaveRefs.NONE` (or `False`) for the root only.
- **Embedding policy** (`core_10x/traitable.py`, `core_10x/concrete_traits.py`): default wire form is value-driven via `s_embeddable` (body vs compact ID ref), including when the declared type is a wide base such as `Traitable`. **`T.EMBEDDED`** still means a closed embed tree (`embed=True` through nested serialize — nested non-embeddable traitables fail). **`T.NOT_EMBEDDABLE`** opts a traitable trait into refs only. Replaces the short-lived hard rule that forbade embeddable values on every non-embeddable declared type.
...
- **Storage / Ibis**: `TraitableHistory.traitable_get` handles a missing `_collection_name` when stripping the `#history` suffix.
...
### Added
- **`EventBase`** (`core_10x/traitable.py`): base class for event-like traitables with store-side `_at` (`T.TS_TIME`) and `_who` (`T.TS_USER`) traits populated on `save()`. `TraitableHistory` inherits from `EventBase`; `keep_history` is not required on `TraitableHistory` subclasses (it is inherited). Custom event collections can extend `EventBase` for the same store-side field behavior.
...
- **`SaveIfChanged` context manager** (`core_10x/ts_store.py`): auto-saves any `Traitable` whose trait values are assigned inside the `with` block. The yielded tracker exposes `tracked_objects()`. An optional sequence of `Traitable` classes restricts which tracked objects are saved via an `isinstance` check (so subclasses of any listed class are saved as well; non-storable classes raise `RuntimeError`). When `EnvVars.use_ts_store_transactions` is enabled (`XX_USE_TS_STORE_TRANSACTIONS=1`), one transaction per distinct store is opened so that a failing `save()` rolls back all saves in the block atomically. Docs added to `GETTING_STARTED.md`; store-agnostic tests in `core_10x/testlib/ts_store_transaction_tests.py::TestSaveIfChanged` (exercised by `core_10x/unit_tests/test_ts_store.py` and `infra_10x/unit_tests/test_ts_stores.py`).
...
- **`xxcommon` package**: calendar, business-day **rdate**, and **curve** moved out of `core_10x` (see *Changed* / migration).
...
- **`curve.value` float coercion** (`xxcommon/curve.py`): `curve.value` now always returns a `float`; previously it could return an `int` or `numpy` scalar in edge cases.
...
- **Rdate**: Business-day roll rules (preceding, following, modified preceding/following) and calendar-aware date utilities in `xxcommon.rdate`
...
- **Unreleased (since 0.2.2)**: **Test store URI:** `testdb://` → `duckdb://`; `TS_STORE_TYPE.TESTDB` → `TS_STORE_TYPE.DUCKDB`. **`TraitableCli` option syntax:** `option=value` → `--option value` (booleans: `--flag` / `--no-flag`). **`ID.__hash__`:** now includes `collection_name` — IDs with the same value in different collections are no longer equal in sets/dicts. **RT-only traitables:** `_collection_name` is not a trait; instance access raises `AttributeError` (was `XNone`). **`T.NOT_EMBEDDABLE`** added (ref-only traitable traits); **`T.EMBEDDED`** retained for closed embed trees.
- **0.2.0**: Imports for **calendar / rdate / curve** moved from `core_10x` to the **`xxcommon`** package (e.g. `xxcommon.rdate`, `xxcommon.curve`, `xxcommon.xxcalendar`). Update any `from core_10x.…` references accordingly. **`core_10x.xinf`**: prefer **`-XInf`** for negative infinity in your code; **`MInf`** is internal to that module (`from core_10x.xinf import *` only exports **`XInf`**).
