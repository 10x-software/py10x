...
def test_publish_trigger_tag_naming():
    ...
    assert VersionHelpers.existing_publish_trigger_tags([f'pre/{KERNEL}0.2.1rc4', f'pre/{KERNEL}0.2.1rc5', f'prod/{KERNEL}0.2.1'], KERNEL) == [
        f'pre/{KERNEL}0.2.1rc4',
        f'pre/{KERNEL}0.2.1rc5',
        f'prod/{KERNEL}0.2.1',
    ]
...
def test_publish_release_tag_selects_latest_rc_or_final():
    parsed = VersionHelpers.parse_pkg_tags([f'{KERNEL}0.2.1rc4', f'{KERNEL}0.2.1rc5', f'{KERNEL}0.2.1'], KERNEL)
...
def test_latest_matching_tag_final_pin_picks_final():
    ...
    p = VersionHelpers.parse_pkg_tags([f'{KERNEL}0.2.0', f'{KERNEL}0.2.1rc1', f'{KERNEL}0.2.1'], KERNEL)
...
def test_pin_strings():
    ...
    assert VersionHelpers.main_forward_pin_from_selection(VersionHelpers.parse_pkg_tags([f'{KERNEL}0.2.1rc19'], KERNEL)) == '>=0.2.1rc19,<0.2.1rc20'
    assert VersionHelpers.main_forward_pin_from_selection(VersionHelpers.parse_pkg_tags([f'{KERNEL}0.2.1'], KERNEL)) == '>=0.2.1,<0.2.2rc1'
...
def test_write_forward_pins_roundtrip(tmp_path):
    ...
    changes = PyProjectHelpers.write_forward_pins(p, {'py10x-kernel': '>=0.2.1,<0.2.2', 'py10x-infra': '>=0.2.1,<0.2.2'})
...
ADMIT_MATRIX = ['0.2.0', '0.2.1.dev1', '0.2.1a1', '0.2.1b1', '0.2.1rc1', '0.2.1rc2', '0.2.1', '0.2.1.post1', '0.2.2.dev0', '0.2.2']
...
COORD_MATRIX = ['1.3.9', '1.4.0rc1', '1.4.0rc2', '1.4.0', '1.4.0.post1', '1.4.1rc1', '1.4.1', '1.5.0rc1']
...
def test_reverse_floor_rc_admits_prereleases():
    ...
    ss = SpecifierSet('>=1.4.0rc2')
    assert ss.prereleases is True
    assert set(ss.filter(COORD_MATRIX)) == {'1.4.0rc2', '1.4.0', '1.4.0.post1', '1.4.1rc1', '1.4.1', '1.5.0rc1'}
...
class TestClassifyInstall:
    def test_editable(self):
        assert InstalledSourceHelpers.classify_install(Path('/proj/py10x'), '') == ('local', Path('/proj/py10x'))
...
class TestDistInfoDirectUrl:
    def test_reads_direct_url_json(self, tmp_path):
        ...
        (dist / 'direct_url.json').write_text('{"url":"https://github.com/org/cxx10x.git","vcs_info":{"vcs":"git"}}')
...
class TestPyPIHelpers:
    def test_parse_released_versions_drops_unparseable(self):
        body = '{"releases": {"0.2.0": [], "0.2.1rc1": [], "0.2.1": [], "garbage": []}}'
...
class TestSelectRunForTag:
    RUNS = [
        {'head_branch': 'pre/v0.2.1', 'created_at': '2026-01-02T00:00:00Z', 'status': 'completed', 'conclusion': 'success', 'html_url': 'u-new'},
        {'head_branch': 'pre/v0.2.1', 'created_at': '2026-01-01T00:00:00Z', 'status': 'completed', 'conclusion': 'failure', 'html_url': 'u-old'},
        {'head_branch': 'prod/v0.3.0', 'created_at': '2026-01-03T00:00:00Z', 'status': 'in_progress', 'conclusion': None, 'html_url': 'u-other'},
    ]
...
class TestRunState:
    def test_completed_reports_conclusion(self):
        assert GitHubHelpers.run_state({'status': 'completed', 'conclusion': 'failure'}) == 'failure'

    def test_in_flight_reports_status(self):
        assert GitHubHelpers.run_state({'status': 'in_progress', 'conclusion': None}) == 'in_progress'
...
class TestPublishWorkflowState:
    ...
    def test_local_only_release(self):
        state, url = GitHubHelpers.publish_workflow_state(self.RUNS, 'pre/v0.2.1', release_on_origin=False, trigger_on_origin=False)
        assert state == 'not pushed to origin'
        assert url == ''

    def test_release_on_origin_but_trigger_local(self):
        state, url = GitHubHelpers.publish_workflow_state(self.RUNS, 'pre/v0.2.1', release_on_origin=True, trigger_on_origin=False)
        assert state == 'publish trigger not on origin (re-run with --push)'
        assert url == ''

    def test_both_on_origin_reports_workflow(self):
        state, url = GitHubHelpers.publish_workflow_state(self.RUNS, 'pre/v0.2.1', release_on_origin=True, trigger_on_origin=True)
