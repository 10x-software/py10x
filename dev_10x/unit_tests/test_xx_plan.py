"""Plan-level tests for `dev_10x.xx_plan.plan_pre_batch` (pure, in-memory, no git).

Asserts the declarative batch-formation rules across the combinatorial space: first-rc vs iterate,
changed vs unchanged package sets, and the pin-lag trigger (a sibling re-cut, or a stale forward
pin, forces a core re-cut). See `dev_10x/docs/rc-branch-promotion.md` (Batch formation).
"""
from __future__ import annotations

from dev_10x.xx_helpers import VersionHelpers
from dev_10x.xx_plan import PkgInput, plan_pre_batch, plan_prod_batch
...
def test_first_rc_for_all_packages_coordinates():
    ...
    assert plans["py10x-kernel"].reverse_test_pin == "py10x-core>=0.0.1rc1"
    assert plans["py10x-infra"].reverse_test_pin == "py10x-core>=0.0.1rc1"
...
def test_rc_iterate_bumps_next_rc_number():
    ...
    assert plans["py10x-kernel"].reverse_test_pin == "py10x-core>=0.2.1rc2"
...
def test_changed_core_only_pins_unchanged_siblings_to_their_finals():
    ...
    assert plans["py10x-core"].recut
    assert plans["py10x-core"].forward_pins == {
        "py10x-kernel": "==1.4.0", "py10x-infra": "==0.9.0"}
    for s in ("py10x-kernel", "py10x-infra"):
        assert not plans[s].recut
        assert plans[s].reverse_test_pin is None
...
def test_sibling_rc_forces_core_recut_even_if_core_unchanged():
    ...
    assert plans["py10x-kernel"].recut and plans["py10x-kernel"].version == "1.4.1rc1"
    assert plans["py10x-core"].recut                                     # forced by the pin lag
    assert plans["py10x-core"].version == "0.2.1rc2"
    assert plans["py10x-core"].forward_pins == {
        "py10x-kernel": "==1.4.1rc1", "py10x-infra": "==0.9.0"}
    assert plans["py10x-kernel"].reverse_test_pin == "py10x-core>=0.2.1rc2"
    assert not plans["py10x-infra"].recut                                # unchanged sibling stays put
...
def test_stale_forward_pin_alone_forces_core_recut():
    ...
    assert not plans["py10x-kernel"].recut
...
def test_nothing_changed_skips_all():
    ...
    assert not plans["py10x-core"].recut and plans["py10x-core"].skip_reason
    assert not plans["py10x-kernel"].recut
...
def test_in_sync_pins_and_no_changes_is_full_skip():
    """Core's pins already match the siblings' coordinated versions and nothing changed -> no re-cut."""
    plans = plan_pre_batch([
        _inp("py10x-core", True, CORE, ["v0.2.0"], False,
             current_forward={"py10x-kernel": "1.4.0"}),
        _inp("py10x-kernel", False, KERNEL, [f"{KERNEL}1.4.0"], False),
...
def test_prod_promotes_rcs_to_finals_and_coordinates():
    ...
    core = plans["py10x-core"]
    assert core.promote and core.target == "0.2.1" and core.tag == "v0.2.1" and core.branch == "prod"
    assert core.forward_pins == {"py10x-kernel": "==1.4.1", "py10x-infra": "==0.9.1"}
    assert core.dev_pins["py10x-kernel"] == VersionHelpers.dev_pin("1.4.1", "1.4.2")   # main epilogue
    assert plans["py10x-kernel"].tag == "py10x-kernel-v1.4.1"
    assert plans["py10x-kernel"].branch == "prod/py10x-kernel"
    assert plans["py10x-kernel"].test_pin == "py10x-core>=0.2.1"
    assert plans["py10x-infra"].target == "0.9.1" and plans["py10x-infra"].test_pin == "py10x-core>=0.2.1"
...
def test_prod_skips_packages_whose_latest_tag_is_final():
    ...
    assert not plans["py10x-core"].promote and plans["py10x-core"].skip_reason
    assert plans["py10x-kernel"].promote and plans["py10x-kernel"].tag == "py10x-kernel-v1.4.1"
    # core not promoted -> no released core to point the sibling test group at
    assert plans["py10x-kernel"].test_pin is None
...
def test_prod_nothing_to_promote_skips_all():
    ...
    assert all(not p.promote for p in plans.values())
