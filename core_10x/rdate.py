...
class RDate(Nucleus):
    ...
    def __init__(self, symbol_or_frequency = None, count: int = None):
        if isinstance(symbol_or_frequency, str):
            ...
            try:
                ...
                raise AttributeError( f"Invalid tenor '{symbol}'" )
            ...
            assert False, f'symbol_or_frequency must be either RDate, str or TENOR_FREQUENCY'
    ...
    def conversion_freq_multiplier(self, other_freq: TENOR_FREQUENCY) -> float:
        ...
        if not multiple:
            raise ValueError( f'cannot get a conversion multiple from {TENOR_FREQUENCY.labelFromValue( my_freq )} to {TENOR_FREQUENCY.labelFromValue( other_freq )}' )
    ...
    def equate_freq(self, other) -> tuple:
        if isinstance(other, str):
            try:
                ...
                raise ValueError(f'{other} - failed to convert to RDate')

        if not isinstance(other, RDate):
            raise ValueError(f'other must be either RDate or convertible str')
        ...
        if int(mult) >= 1:
            return (
                RDate(symbol_or_frequency = other.tenor, count = self.count * int(mult)),
                other
            )

        return (
            self,
            RDate(symbol_or_frequency = self.tenor, count = other.count * int(1 / mult))
        )
    ...
    def __add__(self, other) -> 'RDate':
        return self.multadd(1., other, freq_to_try_for_fract_count = FREQUENCY_TABLE[self.tenor].MIN_CONV_FREQ)
