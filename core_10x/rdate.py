...
class RDate(Nucleus):
    ...
    def equate_freq(self, other: RDate) -> tuple:
        ...
        if mult_i >= 1:
            return (
                RDate(freq = other.freq, count = self.count * mult_i),
                other
            )

        return (
            self,
            RDate(freq = self.freq, count = other.count * int(1 / mult))
        )
    ...
    def __add__(self, other) -> RDate:
        return self.multadd(1., other, freq_to_try_for_fract_count = FREQUENCY_TABLE[self.freq][TENOR_PARAMS.MIN_FREQUENCY])
